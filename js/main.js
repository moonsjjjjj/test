...
//네비 버튼을 눌렸을때 발생 조건에 따라서
//함수를 호출합니다

$('.navi>li').on('click', function (event) {
	event.preventDefault();
	//예 현재는 5번이 보이고 저는 2번을 누르는 상태
	//현재 활성화클래스가 있는 인덱스를 구합니다.
	if (enable_event) {
		enable_event = false;
	}
	let current_index = $('.panel>li').filter('.on').index();
	...
	//민약 활성화인덱스와 타겟인덱스가 같으면? 아무일도 일어나지 않음
	if (active_index == current_index) {
		enable_event = true;
		return;
	}
	//만약 활성화 인덱스가 타겟인덱스보다 크다면? prev
	if (active_index > current_index) {
		show_next(active_index);
	}
	//만약 활성화 인덱스가 타겟인덱스보다 작다면?next
	if (active_index < current_index) {
		show_prev(active_index);
	}
});
...
$('#start').on('click', function () {
	//html태그속겅을 변경하는 방법 attr()<-제이쿼리
	//바닐라 자바스크립트에서는 겟어트리뷰트. 셋어트리뷰트

	$('#start').attr('disabled', true);
	$('#stop').attr('disabled', false);
	//3초마다 함수를 반복적으로 ㅛㅣㄹ행하도록 web api에 요청하는 것.
